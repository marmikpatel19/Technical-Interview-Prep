class LongestIncreasingSubsequence {
    ...
    // The LIS at each index is memoized
    // Dispite this being memoization, we start bottom-up, where the base-case
    // is the last index, since that is gaurenteed to have LIS of 1.

    // time: O(n^2), space: O(1)
    public int lengthOfLIS(int[] nums) {
        int[] LISs = new int[nums.length]; // memo of LIS at each index
        int maxLength = 1;
        ...
        for (int i = nums.length - 2; i >= 0; i--) {
            ...
            // iterate through all following elements to find the LISs for current DP case
