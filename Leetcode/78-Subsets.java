...
class Subsets {
    ...
    // Time: O(n*2^n) -- at each number, there are two choices: to include that
    // number in the currSubset or not to inlcude it
    ...
    private void findSubsets(int[] nums, int currIndex, List<Integer> currSubset, List<List<Integer>> subsets) {
        ...
        // start at currIndex to exhaust all the combinations concerning a particular
        // element and disclude it from
        // future iterative calls, so that it does not appear again (prevent
        // duplication)
