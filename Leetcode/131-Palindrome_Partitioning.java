...
class PalindromePartitioning {
     ...
     * Backtracking:
     * 
     * For any given string, we backtrack on all the possible strings start from the
     * start index
     * and end at each following index. For example, "aab" is broken into three
     * backtracking
     * calls: "a", "aa", and "aab". As a second example, "lamp" is broken into: "l",
     * "la",
     * "lam", and "lamp".
     * 
     * At each call, the string is checked to be a palindrome. This is the condition
     * to
     * determine whether to backtrack--if the current string is not a palindrome,
     * backtrack.
     * If the end of string is reached, also backtrack.
     * 
     * Repeat the backtracking call structure defined above, just now starting at
     * the next index.
     * For instance, with the above example:
     * - The "a" call will work with the string starting at index 1, "ab", and break
     * that
     * down into the calls for "a" and "ab"
     * - The "aa" call will work with the string at index 2, "b", and break that
     * into the call
     * "b"
     ...
     * - The "b" call succeeds and make a call out of bounds which eventually
     * backtracks
     * 
     * - The "b" call succeeds and make a call out of bounds which eventually
     * backtracks
     * 
     * See visualized tree for this example here:
     ...
     * Time Complexity:
     * O(N*2^N)
     * For each n characters there are two choices, two continue recursing or
     * backtrack. At each
